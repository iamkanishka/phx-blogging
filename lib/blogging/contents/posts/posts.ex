  ...
  base_query =
    fn published_only ->
      from p in Post,
        ...
        left_join: cc in subquery(comments_count_query), on: cc.post_id == p.id,
        left_join: like_r in subquery(reaction_queries["like"]), on: like_r.post_id == p.id,
        left_join: love_r in subquery(reaction_queries["love"]), on: love_r.post_id == p.id,
        left_join: wow_r in subquery(reaction_queries["wow"]), on: wow_r.post_id == p.id,
        left_join: laugh_r in subquery(reaction_queries["laugh"]), on: laugh_r.post_id == p.id,
        left_join: sad_r in subquery(reaction_queries["sad"]), on: sad_r.post_id == p.id,
        left_join: angry_r in subquery(reaction_queries["angry"]), on: angry_r.post_id == p.id
    end

  # Apply select and pagination
  select_query =
    fn published_only ->
      from [p, cc, like_r, love_r, wow_r, laugh_r, sad_r, angry_r] in base_query.(published_only),
        select: %{
          p: p,
